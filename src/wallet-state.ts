// Wallet sync-state persistence. Without it, every `npm run deploy` /
// `npm run cli` re-syncs each child wallet from seed — minutes per run on
// preview. The SDK exposes serializeState()/restore() on each child wallet;
// wallet.ts is the glue, this file is the on-disk format underneath.
